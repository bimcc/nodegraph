 ...
 * @returns
 */
export const isInBox = (position, box) => {
 ...
 * @returns
 */
export const normalizeVertor2 = ({ x, y }) => {
    return {
 ...
 */
export const getVerticalVector2 = ({ x, y }) => {
    let a = 0;
    let b = 0;
    if (y !== 0) {
        a = 1; // 设这个垂直向量x=1;
 ...
 */
export const getHeaderNode = (nodes) => {
    const roots = [];
    const _nodes = [];
    for (let nr of nodes) {
        _nodes.push(nr.node);
    ...
    for (let nr of nodes) {
        ...
        for (let input of node.inputs) {
            if (!input.link)
                continue;
            if (_nodes.indexOf(input.link.originNode) === -1)
                continue;
            isRoot = false;
            break;
        }
 ...
 * @returns
 */
export const getTailNode = (nodes) => {
    const roots = [];
    const _nodes = [];
    for (let nr of nodes) {
        _nodes.push(nr.node);
 ...
 */
export const isContinuousNodes = (nodes) => {
    if (nodes.length === 1)
        return false;
    ...
    for (let nr of nodes) {
        ...
        for (let output of node.outputs) {
            for (let link of output.link) {
                if (nodes.indexOf(link.targetNode.render) !== -1) {
                    isContinuous = true;
                    break;
                }
                if (isContinuous)
        ...
        for (let input of node.inputs) {
            if (input.link) {
                if (nodes.indexOf(input.link.originNode.render) !== -1) {
                    isContinuous = true;
                    break;
                }
            }
