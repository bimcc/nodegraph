...
import { IFunction, INodeRender } from "../../interfaces";
import { GraphEventTypes, NodeRenderEvents } from "../../types";
import { AppendCss, wait } from "../../Utils";
import { NativeDiv, NativeButton, NativeSpan } from "../";
...
export class NodePanel {
  ...
  body: {
    ...
    title: NodePanelInput,
    // indexLabel: NodePanelInput,
    idLabel: NodePanelInput,
    props: Array<NativeDiv | NodePanelInput>,
  };
  footer: {
    root: NativeDiv,
    deleteBtn: NativeButton,
    cancelBtn: NativeButton
  }

  isShow: boolean = false;

  onNodeRefresh: IFunction | null = null;
  ...
  refresh(nodeRender: INodeRender) {
    ...
    this.root.setStyle({
      backgroundColor: nodeColor ?? '#4b4b4b',
    })

    this.title.root.setStyle({
      backgroundColor: nodeTitleColor ?? 'rgb(54 54 54)',
    })

    this.body.idLabel.styles.fontColor = nodeFontColor ?? '#ffffff';
    this.body.idLabel.styles.backColor = nodeColor ?? '#4b4b4b';
    // this.body.indexLabel.styles.fontColor = nodeFontColor ?? '#ffffff';
    // this.body.indexLabel.styles.backColor = nodeColor ?? '#4b4b4b';
    this.body.title.styles.fontColor = nodeFontColor ?? '#ffffff';
    this.body.title.styles.backColor = nodeColor ?? '#4b4b4b';
    ...
    for (let key in node.properties) {
      ...
      switch (typeof value) {
        case 'string':
          ...
          pis.styles.backColor = nodeColor ?? '#4b4b4b';
          pis.styles.fontColor = nodeFontColor ?? '#ffffff';
          pis.refresh();
          pis.onChange((value) => {
        ...
        case 'number':
          ...
          pi.styles.backColor = nodeColor ?? '#4b4b4b';
          pi.styles.fontColor = nodeFontColor ?? '#ffffff';
          pi.refresh();
          pi.onChange((value) => {
            node.setProperty(key, value);
          });
    ...
    if (render.node.inputs.length) {
      ...
      for (let index = 0; index < render.node.inputs.length; index++) {
        ...
        pi.onChange((value) => {
          input.label = value
          input.node.render?.getInput(index)?.refresh()
        })
        this.body.props.push(pi)
      }
    }
    if (render.node.outputs.length) {
      ...
      for (let index = 0; index < render.node.outputs.length; index++) {
        ...
        pi.onChange((value) => {
          input.label = value
          input.node.render?.getOutput(index)?.refresh()
        })
        this.body.props.push(pi)
      }
    }
